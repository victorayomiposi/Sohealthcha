    ...
    <div class="container">
        <div class="col-sm-12">
            <div class="card  text-center" style="border-radius: 10px">
                <div class="card-body">
                    <h5 class="card-title text-success" style="font-weight: 800">{{ $name->name }} Payment
                        {{ $category }}</h5>
                    <div class="card-body">
                        <form action="{{ route('payment.department.store') }}" method="post">
                            @csrf
                            <div class="row">
                                <div class="col-sm-4">
                                            ...
                                            @foreach ($payments as $paymenttype)
                                                @php
                                                    $department = DB::table('course_selection')
                                                        ...
                                                        ->first();
                                                @endphp
                                                <option value="{{ $paymenttype->id }}">{{ $department }}
                                                </option>
                                            @endforeach
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#paymenttype').change(function() {
                ...
                if (paymentId) {
                    $.get('/get-payment-amount-for-school/' + paymentId, function(data) {
